...
uses
  Windows, WinSvc, SysUtils, TlHelp32, PsAPI, UnitFunctions, UnitRegistryEditor;
...
procedure CloseButton(Handle: HWND; Disable: Boolean);
...
var
  ...
  DisplayName, Version, UninstallCmd, SilentUninstallCmd, Publisher: string;
  pSize: Integer;
begin
  Result := '';
  ...
  begin
    ...
    SilentUninstallCmd := GetValue('SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\'+ lpName, 'QuietUninstallString');
    ...
    begin
      ...
      Result := Result + ByteSize(pSize) + '|';
      Result := Result + Publisher + '|';
      Result := Result + UninstallCmd + '|';
      Result := Result + SilentUninstallCmd + '|' + #13#10;
    end;
...
begin
  ...
  try
    ...
    CloseServiceHandle(SCManager);
    DeleteRegkey('HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\', ServiceName, False);
  end;
end;
...
function HandleToPid(Handle: integer): integer; var PID: dword;
begin
