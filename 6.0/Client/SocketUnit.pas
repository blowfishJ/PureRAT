...
begin
  ...
  Result := Winsock.Connect(FSocket, SockAddrIn, SizeOf(SockAddrIn));
  if Result < 0 then FConnected := False else FConnected := True;
end;
...
begin
  ...
  begin
     ...
     if Result = 0 then Disconnect;
     if Result = SOCKET_ERROR then
     begin
       Result := WSAGetLastError;
       if Result = WSAEWOULDBLOCK then Result := 0 else Disconnect;
     end;
  end;
end;
...
begin
  ...
  begin
    ...
    if (ErrorCode = WSAEWOULDBLOCK) then Result := -1 else Disconnect;
  end;
end;
