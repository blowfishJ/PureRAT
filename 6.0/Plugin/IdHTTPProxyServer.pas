...
// Procs 
procedure Register;
function StartHttpProxy(port: integer; Mutex: string): boolean;
...
function StopProxy(Mutex: string): boolean;
var
  TempMutex: THandle;
begin
  result := true;
  TempMutex := CreateMutex(nil, False, pchar(Mutex));
...
begin
  ...
  while StopProxy(Mutex) = False do ProcessMessages;
